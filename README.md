...
- **WiFi & Web-интерфейс:**
  ...
  - OTA-обновление прошивки по воздуху (`v0.1.6-alpha`) без кабеля (`/update`).
...
- **`bike_prototype_simple.ino`** — Оптимизированная стабильная версия: throttle-by-wire с PAS, приоритет тормоза, свет/звук передней группы, WiFi-настройки, телеметрия. **Текущая версия: v0.1.6-alpha**
...
- Добавить датчик скорости колеса или другой надёжный датчик движения.
- После появления датчика автоматически определить реальный порог напряжения, при котором велосипед начинает двигаться, и полезный максимальный уровень ручки газа.
- Сохранять результаты такой калибровки отдельно от электрической калибровки диапазона ручки газа.
